...
- [x] 3.3 Implement the AI chat runner that drives the existing runtime kernel.
- [x] 3.4 Persist full diagnostic JSONL logs outside the sandbox, including inputs and outputs without secrets.

## 4. CLI And Scripts

- [x] 4.1 Extend the CLI with `ai run` and result-only output.
- [x] 4.2 Extend the CLI with `ai monitor` real-time status output.
- [x] 4.3 Add one-click PowerShell scripts for running and monitoring.
- [x] 4.4 Add log directory options to run and monitor commands and scripts.

## 5. Verification

- [x] 5.1 Add tests for configuration loading, sandbox cleanup, result-only output, flow events, and persistent diagnostic logs.
- [x] 5.2 Run OpenSpec validation, automated tests, compile check, and hardcoding scan.
